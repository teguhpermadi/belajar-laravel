        ...
        <div class="col-12">
            <x-adminlte-card title="Foto Profil" icon="fas fa-lg fa-portrait">
                ...
                <form enctype="multipart/form-data" action="{{ route('update_avatar') }}" method="POST">
                    ...
                    <br>
                    <x-adminlte-button class="d-flex" theme="primary" label="simpan" icon="fas fa-sign-in"
                        type="submit" />
                </form>
        ...
        <div class="col-12">
            <x-adminlte-card title="Identitas" icon="fas fa-lg fa-user" collapsible>
                <form action="{{ route('update_profile') }}" method="post">
                    ...
                    <x-adminlte-button class="d-flex ml-auto" theme="primary" label="simpan" icon="fas fa-sign-in"
                        type="submit" />
                </form>
            </x-adminlte-card>
        </div>
            ...
            <x-adminlte-card title="Email & Password" icon="fas fa-lg fa-key" collapsible>
                <form action="{{ route('update_emailpassword') }}" method="post">
                    ...
                    <x-adminlte-input name="username" label="Username" placeholder="username"
                        value="{{ $user->username }}" />
                    <x-adminlte-input name="email" type="email" label="Email" placeholder="email" value="{{ $user->email }}" />
                    <x-adminlte-input name="current_password" label="Password Lama" type="password" placeholder="password lama" />
                    <x-adminlte-input name="new_password" label="Password baru" type="password" placeholder="password baru" />
